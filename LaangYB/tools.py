...
@Laang(["cinfo", "chatinfo", "ginfo"])
async def chatinfo_handler(client: Client, message: Message):
    Laang = await eor(message, "<i>Processing...</i>")
    try:
...
"""
        ...
        if photo_id:
            ...
            await Laang.edit(out_str, disable_web_page_preview=True)
    except Exception as e:
        return await Laang.edit(f"<b>INFO:</b> <code>{e}</code>")
