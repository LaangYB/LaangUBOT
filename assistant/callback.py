    ...
    try:
        ...
        if HOSTED_ON == "Heroku":
            ...
            if not dotenv.get_key(path, vars):
                ...
                os.execvp(sys.executable, [sys.executable, "-m", "pyALaang"])
            else:
                pass
    except Exception as e:
        return await cq.message.reply("[ERROR]\n{}\nSilahkan Teruskan Pesan Ini Ke @ybtraviss".format(str(e)))
    ...
    try:
        ...
        if HOSTED_ON == "Heroku":
            ...
            if not dotenv.get_key(path, vars):
                dotenv.set_key(path, vars, string_session)
                logs.info(f"Berhasil Menambahkan {vars}")
                os.execvp(sys.executable, [sys.executable, "-m", "pyLaang"])
            else:
                pass
    except Exception as e:
        return await cq.message.reply("[ERROR]\n{}\nSilahkan Teruskan Pesan Ini Ke @ybtraviss".format(str(e)))
